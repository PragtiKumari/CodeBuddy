// hackerrank.js - HackerRank Content Script
(function () {
  // Configuration
  const CONFIG = {
    BACKEND_URL: "http://localhost:3000/api/hint",
    BUTTON_ID: "codebuddy-hint-button-hr",
  ...
  const isProblemPage = () => {
    // HackerRank problem URLs: /challenges/problem-name/, /contests/contest-name/challenges/problem-name/
    return (window.location.pathname.includes("/challenges/") || 
            window.location.pathname.includes("/challenge/")) &&
           !window.location.pathname.endsWith("/challenges/") &&
           !window.location.pathname.endsWith("/challenge/");
  };

  const extractProblemTitle = () => {
    // HackerRank specific selectors
    const selectors = [
      'h1.ui-icon-label',
      '.challenge-title h1',
      'h1[class*="challenge"]',
    ...
    for (const selector of selectors) {
      ...
      if (element && element.textContent.trim()) {
        ...
        // Clean up HackerRank specific formatting
        title = title.replace(/^\d+\.\s*/, '').trim();
        title = title.replace(/\s*\|\s*HackerRank/gi, '').trim();
        title = title.replace(/\s*-\s*HackerRank/gi, '').trim();
        title = title.replace(/\s*\(Easy\)|\s*\(Medium\)|\s*\(Hard\)/gi, '').trim();
        
        // Remove common HackerRank prefixes
        title = title.replace(/^(Problem:\s*|Challenge:\s*)/gi, '').trim();
    ...
    const challengeIndex = urlParts.findIndex(part => part === 'challenges' || part === 'challenge');
    if (challengeIndex !== -1 && challengeIndex < urlParts.length - 1) {
      const problemSlug = urlParts[challengeIndex + 1];
      if (problemSlug) {
  ...
  const createHintButton = () => {
    ...
    // HackerRank-specific styling (green theme)
    ...
    button.addEventListener("mouseenter", () => {
      button.style.backgroundColor = "#00a041";
      button.style.transform = "translateY(-2px)";
      button.style.boxShadow = "0 6px 16px rgba(0, 200, 81, 0.4)";
    });

    button.addEventListener("mouseleave", () => {
      button.style.backgroundColor = "#00c851";
      button.style.transform = "translateY(0)";
      button.style.boxShadow = "0 4px 12px rgba(0, 200, 81, 0.3)";
    });
  ...
  const showHintModal = (hint, source) => {
    ...
    const existingModal = document.getElementById("codebuddy-hint-modal-hr");
    if (existingModal) {
      existingModal.remove();
    }

    const modalContainer = document.createElement("div");
    modalContainer.id = "codebuddy-hint-modal-hr";
    ...
    const modalHTML = `
      <div class="modal-overlay">
        <div class="modal-content">
          ...
          <h3 class="modal-title">💡 Hint for HackerRank Challenge</h3>
  ...
  const injectHintButton = () => {
    ...
    if (!problemTitle) {
      console.log("⚠️ HackerRank: Could not extract problem title, retrying...");
      return;
    }

    console.log("✅ CodeBuddy HackerRank: Injecting hint button for:", problemTitle);
  ...
  const initializeExtension = () => {
    if (isProblemPage()) {
      console.log("✅ CodeBuddy HackerRank: Problem page detected on load!");
  ...
  console.log("🚀 CodeBuddy HackerRank: Extension initialized!");
})();