(function () {
  ...
  const isProblemPage = () => {
    return window.location.pathname.includes("/problem/") || 
           ...
           window.location.pathname.includes("/contest/") && window.location.pathname.includes("/problem/");
  };

  const extractProblemTitle = () => {
    // Codeforces problem title selectors
    const selectors = [
    ...
    for (const selector of selectors) {
      ...
      if (element && element.textContent.trim()) {
        ...
        // Clean up the title - remove problem codes and extra text
        title = title.replace(/^[A-Z]\.\s*/, '').trim(); // Remove "A. " prefix
        title = title.replace(/^\d+[A-Z]\.\s*/, '').trim(); // Remove "1A. " prefix
    ...
    const problemIndex = urlParts.findIndex(part => part === 'problem');
    if (problemIndex >= 0 && problemIndex < urlParts.length - 1) {
      const problemCode = urlParts[problemIndex + 1];
      return problemCode.split('-').map(word => 
  ...
  const createHintButton = () => {
    ...
    // Enhanced styling for Codeforces
  ...
  const showHintModal = (hint, source) => {
    ...
    // Create Shadow DOM to isolate from Codeforces CSS
    ...
    const modalHTML = `
      <div class="modal-overlay">
        <div class="modal-content">
          <div class="platform-badge">Codeforces</div>
  ...
  const fetchHint = async (problemTitle) => {
    ...
    try {
      ...
      if (data?.hint) {
        ...
        showHintModal(
          "No specific hint available for this problem. Consider the time complexity requirements and think about mathematical properties or algorithms that might apply.",
          "fallback"
        );
      }
    } catch (error) {
      ...
      showHintModal(
        "Unable to fetch hint right now. For Codeforces problems, often think about number theory, graph algorithms, or mathematical patterns.",
        "offline"
      );
    } finally {
      // Restore button
  ...
  const injectHintButton = () => {
    ...
    if (!problemTitle) {
      console.log("⚠️ CodeBuddy: Could not extract Codeforces problem title, retrying...");
      return;
    }

    console.log("✅ CodeBuddy: Injecting hint button for Codeforces problem:", problemTitle);
  ...
  const initializeExtension = () => {
    if (isProblemPage()) {
      console.log("✅ CodeBuddy: Codeforces problem page detected!");
  ...
  console.log("🚀 CodeBuddy: Codeforces extension initialized!");
})();